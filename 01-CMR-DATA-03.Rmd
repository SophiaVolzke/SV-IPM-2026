...
Import Data
```{r}
dsights <- read_csv("TAGS_ELLIE_SIGHTINGS6_Share.csv", na = "n/a")
```
...
dsights <- dsights%>%
  ...
  mutate(BAD= if_else(MASTER_TAG_TYPE == "T" & TAG_Season > 1980, TRUE, FALSE)) %>%
  filter(BAD==FALSE)%>%
  mutate(BAD= if_else(MASTER_TAG_TYPE == "#N/A" & TAG_Season > 1980, TRUE, FALSE)) %>%
  filter(BAD==FALSE)
head(dsights)
```
...
# Capture Histories1 (CH)

Make encounter history data frame
These steps are documented in numbers to check the data comes through correctly.
```{r}
...
```{r}
# Assuming df is the original data frame with columns 'SEAL_ID', 'Season', and 'State'
